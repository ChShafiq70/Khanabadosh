...
const Header = () => {
  ...
  return (
    <header className="">
      <div className="bg-[#B22222] text-white text-center md:text-end py-2  md:px-30 text-lg z-10">
        Questions? Call now 1-844-344-4666
      </div>
      <nav className="bg-[#F6F6F6] py-4 px-4 md:px-30 flex justify-between items-center relative z-10">
        <div className="flex items-center space-x-2">
          <Image src="/images/logo.png" alt="Shield My Life" width={250} height={100} />
        </div>
        ...
        <Link href="/">
  <button className="hidden md:block bg-[#B22222] text-white py-2 px-4 rounded-lg hover:bg-red-600">
    Contact Us
  </button>
</Link>
      ...
      <div
        className={`z-10 fixed inset-0  bg-gray-100 bg-opacity-90 transition-opacity duration-800 ${menuOpen ?  "translate-x-0" : "-translate-x-full"} transition-transform ease-in-out duration-700 md:hidden`} 
        onClick={() => setMenuOpen(false)}
      ></div>
      <div
        className={`z-10 fixed top-0 left-0 w-80 h-full bg-white shadow-2xl p-6 transform ${menuOpen ? "translate-x-0" : "-translate-x-full"} transition-transform ease-in-out duration-2000 md:hidden`}
      >
